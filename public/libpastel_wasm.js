...
var ASM_CONSTS = {
  1349232: () => { if (!FS.analyzePath('/wallet_data').exists) { FS.mkdir('/wallet_data'); FS.mount(IDBFS, {}, '/wallet_data'); console.log('/wallet_data directory created and IDBFS mounted.'); } else { console.log('/wallet_data directory already exists, no need to mount IDBFS.'); } FS.syncfs(true, function(err) { if (err) { console.error('Error syncing file system:', err); } else { console.log('File system synced successfully.'); } }); },  
 1349654: () => { FS.syncfs(function(err) { if (err) { console.error('Error syncing file system:', err); } else { console.log('File system synced successfully.'); } }); },  
 1349805: () => { return Module.getRandomValue(); },  

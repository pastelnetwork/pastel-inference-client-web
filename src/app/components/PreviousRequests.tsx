...
const ContentRenderer = ({ content }: { content: string }) => {
  ...
  try {
    ...
    // If it's not JSON, just use the original content
  ...
  // If not markdown, render as pre-formatted text
  return <pre className="whitespace-pre-wrap">{processedContent}</pre>;
};
...
export default function PreviousRequests() {
  ...
  const getPromptPreview = (request: InferenceRequest) => {
    if (request.selectedInferenceType === "text_to_image") {
      ...
      return (
        request.inputFields.question?.substring(0, 40) ||
        "No question available"
      );
    } else if (request.selectedInferenceType === "embedding_document") {
      return (
        request.inputFields.document_file_name || "No document name available"
      );
    } else if (request.selectedInferenceType === "embedding_audio") {
      return (
        request.inputFields.audio_file_name || "No audio file name available"
      );
    } else {
  ...
  const exportRequests = () => {
    ...
    requests.forEach((request, index) => {
      ...
      if (request.selectedInferenceType === "text_to_image") {
        ...
        let content = request.inferenceResultsDecoded;
        try {
          content = JSON.parse(request.inferenceResultsDecoded);
        } catch {
  ...
  const renderRequestPreview = (request: InferenceRequest) => {
    ...
    if (request.selectedInferenceType === "text_to_image") {
      content = (
        <div className="mb-4">
          <img
            src={request.inferenceResultsDecoded}
            alt="Generated Image"
            style={{ maxWidth: "100%", maxHeight: "400px" }}
          />
          <p className="text-sm italic m-2">
            (This is a compressed version of the original generated image.)
          </p>
        </div>
      );
    } else {
      content = (
        <div className="mb-4">
          <ContentRenderer content={request.inferenceResultsDecoded} />
        </div>
      );
    }
        ...
        <table className="inference-table">
          <tbody>
            ...
            <tr>
              ...
              <td>{request.actualCreditsUsed}</td>
            </tr>
            <tr>
              <th>Remaining Credits</th>
              <td>{request.remainingCredits}</td>
            </tr>
            <tr>
            ...
            <tr>
              <th>Model</th>
              <td>{request.selectedModelCanonicalName}</td>
            </tr>
            <tr>
              <th>Total Time in Seconds to Process Request</th>
              <td>{request.elapsedTimeInSeconds}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 p-4 has-border rounded-xl bg-white shadow-md mt-3">
      <div className="lg:col-span-1 flex flex-col">
        ...
        <div
          ...
          className="bg-gray-100 p-4 rounded-lg overflow-y-auto flex-grow"
          style={{ maxHeight: "1200px" }}
        >
          {requests.length === 0 ? (
            <p className="text-gray-500">
              No saved inference requests to display. Create new ones and they
              will display here when they finish successfully.
            </p>
          ) : (
            requests.map((request, index) => (
              <div
                key={index}
                className={`mb-2 p-2 bg-white rounded-lg flex justify-between items-center cursor-pointer border ${
              ...
              >
                <span>
                  <span className="icon-large">
                    {getInferenceTypeIcon(request.selectedInferenceType)}
                  </span>{" "}
      ...
      <div
        id="requestPreview"
        className="lg:col-span-2 bg-gray-50 p-4 rounded-lg"
      >
        {selectedRequest ? (
